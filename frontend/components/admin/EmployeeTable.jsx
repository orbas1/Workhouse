...
export default function EmployeeTable({ employees, onRefresh }) {
  ...
  return (
    <>
      <Button colorScheme="teal" onClick={openAdd} mb={4}>Add Employee</Button>
        ...
        <Tbody>
          {employees.map(emp => (
            <Tr key={emp.id}>
              ...
              <Td>{emp.email}</Td>
              <Td>
                <IconButton icon={<EditIcon />} mr={2} onClick={() => openEdit(emp)} aria-label="Edit" />
                <IconButton icon={<DeleteIcon />} colorScheme="red" onClick={() => handleDelete(emp.id)} aria-label="Delete" />
              </Td>
            </Tr>
          ))}
        </Tbody>
