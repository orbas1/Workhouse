...
async function acceptJob(jobId, agencyId) {
  ...
  if (job.status !== 'open') throw new Error('Job is not available for acceptance');
...
async function assignJob(jobId, employeeId, agencyId) {
  const job = jobModel.findById(jobId);
  if (!job) throw new Error('Job not found');
  if (job.status !== 'accepted') throw new Error('Job must be accepted before assignment');
  if (job.acceptedBy !== agencyId) throw new Error('Agency not authorized to assign this job');
