const { createDispute, addResponse, findById, findAll } = require('../models/dispute');
...
function listDisputesHandler(req, res) {
  const { userId, disputeeId } = req.query;
  let disputes = findAll();
  if (userId) {
    disputes = disputes.filter((d) => d.userId === userId);
  }
  if (disputeeId) {
    disputes = disputes.filter((d) => d.disputeeId === disputeeId);
  }
  res.json(disputes);
}
...
async function getDisputeDetails(req, res) {
  const { disputeId } = req.params;
  const dispute = findById(disputeId);
  if (!dispute) {
    return res.status(404).json({ error: 'Dispute not found' });
  }
